import type { Client, GatewayIntentsString, Status } from "discord.js";
...
export class Cluster<T extends Client> {
	...
	private _onRequest(message: unknown): unknown {
		...
		if(m.type == "CUSTOM" && this.eventMap.request) {
			...
			const shardPings: { id: number, ping: number, status: Status, uptime?: unknown, guilds: number, members: number }[] = [];
			try {
...
export interface ClusterEventListeners {
