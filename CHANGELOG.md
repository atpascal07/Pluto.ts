...
- **`SELF_DESTRUCT` request handler** now returns a `Promise` to EventManager, meaning the response is only sent *after* cleanup has finished (giving the parent time to receive it before the SIGKILL fallback fires).
